    ...
    User.findOne({ email: emaillowerCase }, (error, userStorage)=>{
        if(error){
            ...
            bcrypt.compare(password, userStorage.password, (bcryptError, check)=>{
                if(bcryptError){ //verifica que si existe un error en la comparacion se envia un error al servidor
                    res.status(500).send({ msg: "Error del servidor" } )
                }else if(!check){ //en caso de que la comparacion sea falsa se envia un error al servidor
                ...
                } else if(!userStorage.active){ //verifica que el atribudo de actividad del usuario sea activo
                    res.status(401).send({ msg: "Usuario inactivo o no autorizado" })
                } else { //esta es la condicion donde si no existe ningun error en la comprobacion se valida el login
                    res.status(200).send({ 
